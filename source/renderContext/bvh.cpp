...
static bool IntersectTriangle(const GfVec3f &rayOrigin, const GfVec3f &rayDir, const GfVec3f &v0, const GfVec3f &v1,
                              const GfVec3f &v2, float &t, float &outU, float &outV)
{
    GfVec3f edge1 = v1 - v0;
    GfVec3f edge2 = v2 - v0;
...
static bool IntersectAABB(const GfVec3f &rayOrigin, const GfVec3f &rayDir, const GfRange3f &range, float &tMinHit)
{
    if (range.IsEmpty())
        return false;
...
void BVH::Build(const VtVec3fArray &points, const VtVec3iArray &indices, const VtVec2fArray &uvs,
                const VtVec3fArray &normals, const VtVec3fArray &colors, const std::vector<int> &materialIndices)
{
    _triangles.clear();
    _nodes.clear();
    ...
    for (size_t i = 0; i < indices.size(); ++i)
    {
        ...
        if (triIdx[0] < 0 || triIdx[0] >= (int)points.size() || triIdx[1] < 0 || triIdx[1] >= (int)points.size() ||
            triIdx[2] < 0 || triIdx[2] >= (int)points.size())
            continue;
...
bool BVH::Intersect(const GfVec3f &rayOrigin, const GfVec3f &rayDir, float &t, GfVec3f &normal, GfVec2f &uv,
                    GfVec3f &smoothNormal, GfVec3f &dpdu, GfVec3f &dpdv, GfVec3f &smoothColor, int &materialIndex) const
{
    if (_nodes.empty())
        return false;
...
bool BVH::_IntersectNode(int nodeIdx, const GfVec3f &rayOrigin, const GfVec3f &rayDir, float &t, GfVec3f &normal,
                         GfVec2f &uv, GfVec3f &smoothNormal, GfVec3f &dpdu, GfVec3f &dpdv, GfVec3f &smoothColor,
                         int &materialIndex) const
{
    const BVHNode &node = _nodes[nodeIdx];
    float          tAabb;
    if (!IntersectAABB(rayOrigin, rayDir, node.bounds, tAabb))
        return false;
    if (tAabb > t)
        return false;
    ...
    {
        bool hitLeft  = _IntersectNode(node.leftChild, rayOrigin, rayDir, t, normal, uv, smoothNormal, dpdu, dpdv,
                                       smoothColor, materialIndex);
        bool hitRight = _IntersectNode(node.leftChild + 1, rayOrigin, rayDir, t, normal, uv, smoothNormal, dpdu, dpdv,
                                       smoothColor, materialIndex);
