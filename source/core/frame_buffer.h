...
namespace Restir
{
    ...
    class FrameBuffer final
    {
      public:
        ...
        [[nodiscard]] gsl::span<unsigned char> Bytes() noexcept
        {
            return _storage;
        }
        [[nodiscard]] gsl::span<const unsigned char> Bytes() const noexcept
        {
            return _storage;
        }
    ...
    template <typename T> gsl::span<T> FrameBuffer::As()
    {
    ...
    template <typename T> gsl::span<const T> FrameBuffer::As() const
    {
        Expects(sizeof(T) == _stride);
        // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
