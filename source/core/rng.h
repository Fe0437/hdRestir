...
public:
    ...
    void ResetSeed(uint32_t seed) noexcept { _seed.seed(seed); }

    [[nodiscard]] int NextInt() noexcept { return _distInt(_seed); }

    [[nodiscard]] uint32_t NextUint() noexcept { return _distUint(_seed); }
    ...
    [[nodiscard]] float NextFloat() noexcept { return _distFloat(_seed); }
