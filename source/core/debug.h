...
#    define DBG_ASSERT(cond, msg)                                                               \
        do {                                                                                    \
            if (!(cond)) {                                                                      \
                ...
                std::string _dbg_message = std::string("[HdRestir] assertion failed: ") +      \
                    (msg) + " (" + _dbg_loc.file_name() + ":" +                                \
                    std::to_string(_dbg_loc.line()) + " in " + _dbg_loc.function_name() + ")";\
...
#else
#  define DBG_ASSERT(cond, msg) \
    do { if (!(cond)) { (void)sizeof(msg); std::abort(); } } while (0)
#endif
...
#    define DBG_UNREACHABLE(msg) \
        do { (void)sizeof(msg); std::unreachable(); } while (0)
#  elif defined(_MSC_VER)
#    define DBG_UNREACHABLE(msg) \
        do { (void)sizeof(msg); __assume(false); } while (0)
#  elif defined(__clang__) || defined(__GNUC__)
#    define DBG_UNREACHABLE(msg) \
        do { (void)sizeof(msg); __builtin_unreachable(); } while (0)
#  else
#    define DBG_UNREACHABLE(msg) \
        do { (void)sizeof(msg); std::abort(); } while (0)
#  endif
#endif
