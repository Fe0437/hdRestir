...
namespace Restir
{
    ...
    namespace
    {
        ...
        void BlitBuffer(gsl::span<PixelT> dst, gsl::span<const PixelT> srcL, gsl::span<const PixelT> srcR, int width,
                        int height, float splitT)
        {
            Expects(width > 0);
            Expects(height > 0);
        ...
        template <typename PixelT>
        void ComposeOptionalBuffer(RenderContext &baseCtx, const RenderContext &leftCtx, const RenderContext &rightCtx,
                                   std::string_view name, float splitT)
        {
        ...
        void ComposeRequestedOutputs(RenderContext &baseCtx, const RenderContext &leftCtx,
                                     const RenderContext &rightCtx, float splitT)
        {
    ...
    SplitScreenCompositor::SplitScreenCompositor(std::unique_ptr<RenderPipeline> left,
                                                 std::unique_ptr<RenderPipeline> right, int leftResolutionLevel,
                                                 int rightResolutionLevel, int leftTargetSamples,
                                                 int rightTargetSamples)
        : _left(std::move(left)), _right(std::move(right)), _leftResolutionLevel(leftResolutionLevel),
          _rightResolutionLevel(rightResolutionLevel), _leftTargetSamples(leftTargetSamples),
    ...
    {
        const int splitX{std::clamp(static_cast<int>(_splitT * static_cast<float>(baseCtx.frame.windowWidth)), 0,
    ...
    void SplitScreenCompositor::blit(gsl::span<GfVec4f> dst, gsl::span<const GfVec4f> srcL,
                                     gsl::span<const GfVec4f> srcR, int width, int height, float splitT)
    {
