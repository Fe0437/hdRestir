...
namespace Restir
{
    ...
    namespace Detail
    {
        ...
        [[nodiscard]] inline bool IsSubset(gsl::span<const std::string> actual, gsl::span<const std::string> possible)
        {
        ...
        template <typename PassT, typename... Args> requires HasStaticPassIo<PassT>
        [[nodiscard]] inline PipelinePassSpec MakePassSpec(Args &&...args)
        {
            auto ctorArgs{std::tuple<std::decay_t<Args>...>{std::forward<Args>(args)...}};
            auto pass{std::apply([](const auto &...unpackedArgs) { return std::make_unique<PassT>(unpackedArgs...); },
                                 ctorArgs)};
            ...
            return PipelinePassSpec{
                ...
                {
                    return std::apply([](const auto &...unpackedArgs) -> std::unique_ptr<RenderPass>
        ...
        CompilePipeline(std::string &&name, const std::vector<std::string> &requestedOutputs,
                        std::vector<PipelinePassSpec> passSpecs)
        {
            ...
            for (std::size_t idx{passSpecs.size()}; idx > 0; --idx)
            {
                ...
                const bool  producesNeededOutput{std::any_of(passSpec.Outputs.begin(), passSpec.Outputs.end(),
            ...
            if (includedPassIndices.empty())
            {
                ...
                DBG_LOG("CompilePipeline: no pass chain can satisfy requested outputs [%s]",
                        requestedOutputList.c_str());
                return pipeline;
            }
    ...
    [[nodiscard]] inline std::unique_ptr<RenderPipeline>
    makeCorePathTracerPipeline(std::string &&name, const PathTracerPipelineSettings &settings)
    {
    ...
    [[nodiscard]] inline std::unique_ptr<RenderPipeline>
    makePathTracerPipeline(std::string &&name = "PathTracer", const PathTracerPipelineSettings &settings = {})
    {
        std::vector<Detail::PipelinePassSpec> passSpecs{};
        passSpecs.push_back(Detail::MakePassSpec<RaycastPass>(settings.OutputNames));
        passSpecs.push_back(Detail::MakePassSpec<PathTracePass>(settings.PathTrace, settings.MaxDepth));
