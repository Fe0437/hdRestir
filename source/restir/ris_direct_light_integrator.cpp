...
namespace Restir
{
    ...
    SampledSpectrum RisDirectLightIntegrator::Li(const ShadingPoint &surface, const IScene &scene, Rng &rng,
    ...
    SampledSpectrum RisDirectLightIntegrator::Li(const ShadingPoint &surface, const IScene &scene, Rng &rng) const
    {
    ...
    RisDirectLightIntegrator::_generateBSDFSamplingCandidates(const ShadingPoint &surface, const IScene &scene,
                                                              Rng &rng, int candidateCount,
                                                              const std::optional<BsdfBounceConnection> &bsdfConnection)
    {
        ...
        if (bsdfConnection.has_value())
        {
            ...
            if (connection.Bounce.BsdfPdf.value > 0.0f)
            {
                ...
                const double misWeight{
                    PowerHeuristic(bsdfContrib.PBsdf, candidateCount, bsdfContrib.PNee, candidateCount)};
                const double targetFunction{
                    SpectrumLuminance(bsdfContrib.Emission * bsdfContrib.ThroughputIntegrandMul, surface.lambda)};
        ...
        for (int i{0}; i < candidatesToGenerate; ++i)
        {
            ...
            if (!sampledConnection.Hit.has_value())
            {
                ...
                const SampledSpectrum  envEmission{
                    RGBToSpectrum(environment->Sample(sampledConnection.Bounce.NextRay.Dir), surface.lambda)};
    ...
    [[maybe_unused]] std::vector<RisDirectLightIntegrator::OptionalRisLightCandidate>
    RisDirectLightIntegrator::_generateNEECandidates(const ShadingPoint &surface, const IScene &scene, Rng &rng,
                                                     const ILightSampler &sampler, int candidateCount,
            ...
            if (skyLight != nullptr)
            {
                ...
                if (lightSample.has_value())
                {
                    ...
                    const float      targetFunction{
                        SpectrumLuminance(nee.Emission * nee.ThroughputIntegrandMul, surface.lambda)};
                    const float  misWeight{(nee.UseMis && useBsdfTechnique)
    ...
    RisDirectLightIntegrator::MISContrib RisDirectLightIntegrator::_evaluateNEE(const ShadingPoint &surface,
                                                                                const ILight       &light,
    ...
    RisDirectLightIntegrator::_evaluateBSDFConnection(const ShadingPoint         &surface,
                                                      const BsdfBounceConnection &connection, const IScene &scene)
    {
        ...
        if (connection.Bounce.ImpossibleNEEConnection || pBsdf <= 0.0f)
        {
            if (!connection.Hit.has_value())
            {
                ...
                return {
                    .Emission =
                        environment ? RGBToSpectrum(environment->Sample(wi), surface.lambda) : SampledSpectrum{},
        ...
        if (hitLight != nullptr)
        {
            ...
            {
                const Pdf areaPdf{
                    hitLight->EvalPdf(surface.hit.Position, wi, std::sqrt(dist2), connection.Hit->Normal)};
