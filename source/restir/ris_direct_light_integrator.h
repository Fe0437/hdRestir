...
class RisDirectLightIntegrator final
...
public:
    ...
    {
        return std::make_unique<RisDirectLightIntegrator>(
            NotNullUniquePtr<ILightSampler>{_sampler->CloneAs()}, _candidateCount);
    }
...
private:
    ...
    struct RISLightCandidate {
        ...
        [[nodiscard]] double GetWeight() const { return risWeight; }
    ...
    struct OptionalRisLightCandidate : std::optional<RISLightCandidate> {
        ...
        [[nodiscard]] double GetWeight() const { return this->has_value() ? this->value().risWeight : 0.0; }
    };
    ...
    [[nodiscard]] static MISContrib _evaluateNEE(
        const ShadingPoint& surface,
        const ILight&       light,
    ...
    [[nodiscard]] static MISContrib _evaluateBSDFConnection(
        const ShadingPoint&      surface,
        const BsdfBounceConnection& connection,
        const IScene&            scene);

    [[nodiscard]] static std::vector<OptionalRisLightCandidate> _generateBSDFSamplingCandidates(
          const ShadingPoint& surface,
        const IScene&       scene,
        Rng&                rng,
        int                 candidateCount,
    ...
    [[nodiscard]] static std::vector<OptionalRisLightCandidate> _generateNEECandidates(
        const ShadingPoint& surface,
        const IScene&       scene,
        Rng&                rng,
