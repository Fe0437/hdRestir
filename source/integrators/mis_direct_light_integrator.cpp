...
SampledSpectrum MisDirectLightIntegrator::Li(
...
MisDirectLightIntegrator::MISContrib MisDirectLightIntegrator::_evaluateNEE(
...
SampledSpectrum MisDirectLightIntegrator::_integrateNEE(
    const ShadingPoint& surface,
    const IScene&       scene,
    Rng&                rng,
...
SampledSpectrum MisDirectLightIntegrator::_integrateBSDFConnection(
    const ShadingPoint&        surface,
    ...
    const IScene&             scene) const
{
    if (connection.Bounce.ImpossibleNEEConnection || connection.Bounce.BsdfPdf.value <= 0.0f) {
        if (!connection.Hit.has_value()) {
            ...
            return (environment != nullptr
                ...
                : SampledSpectrum{0.0f}) * connection.Bounce.ThroughputMul;
        }
    ...
    if (!connection.Hit.has_value()) {
        ...
        if (hitLight != nullptr) {
            ...
            if (cosY > 0.0f) {
                const Pdf areaPdf{hitLight->EvalPdf(surface.hit.Position,
