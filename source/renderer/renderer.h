...
namespace Restir
{
    ...
    class Renderer final
    {
      public:
        ...
        void SetRequestedOutputNames(std::vector<std::string> outputNames)
        {
            _requestedOutputNames = std::move(outputNames);
        }
        [[nodiscard]] gsl::span<const std::string> GetRequestedOutputNames() const
        {
            return _requestedOutputNames;
        }
        [[nodiscard]] static std::vector<std::string>
        CollectRequestedOutputNames(gsl::span<const AovBinding> aovBindings);

        void Render(const IRenderJob &job, IScene &scene);
        void Clear();
