...
namespace Restir
{
    ...
    const detail::Texture *ImageTextureSamplerFactory::_getOrLoad(std::string_view resolvedPath, bool linear) const
    {
        ...
        if (!image)
        {
            DBG_LOG("  Failed to open texture: %.*s", static_cast<int>(resolvedPath.size()), resolvedPath.data());
        ...
        bool      isFloat =
            (format == HioFormatFloat32 || format == HioFormatFloat32Vec2 || format == HioFormatFloat32Vec3 ||
             format == HioFormatFloat32Vec4 || format == HioFormatFloat16 || format == HioFormatFloat16Vec2 ||
        ...
        if (isFloat)
        {
            ...
            if (!image->Read(spec))
            {
                DBG_LOG("  Failed to read texture pixels (float): %.*s", static_cast<int>(resolvedPath.size()),
                        resolvedPath.data());
            }
            else
            {
        ...
        else
        {
            spec.format =
                channels == 4
                    ? (isOriginalSrgb ? HioFormatUNorm8Vec4srgb : HioFormatUNorm8Vec4)
            ...
            if (!image->Read(spec))
            {
                DBG_LOG("  Failed to read texture pixels (unorm8): %.*s", static_cast<int>(resolvedPath.size()),
                        resolvedPath.data());
            }
            else
            {
