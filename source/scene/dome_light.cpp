...
void DomeLight::Prepare()
{
    ...
    if (texPath.GetAssetPath().empty()) return;
...
{
    if (_pixels.empty()) return std::nullopt;
    ...
    if (_totalLuminance > 0) {
        ...
        lightPdf = std::max(lum / (_totalLuminance
...
{
    if (_pixels.empty()) return GfVec3f{0.0f};
