...
void Scene::BuildRenderState(const SceneBuildRenderStateConfig& config, const IRenderJob& job)
{
...

    {
        ...
        for (const auto& [id, _] : _lightInputs) currentLightIds.insert(id);
    ...
    if (config.EnablePhysicalSky) {
        ...
        const GfVec3f sunDir{
            GfVec3f{std::sin(sunAngle) * std::cos(0.5f),
                    std::cos(sunAngle),
                    std::sin(sunAngle) * std::sin(0.5f)}.GetNormalized()
        };
...
{
    if (!_renderState.has_value()) return {};
...
void Scene::_BuildTLAS(RenderState& state, const IRenderJob& job)
{
    state.TlasNodes.clear();
    state.TlasInstanceIndices.clear();
    if (state.Instances.empty() || job.IsCancelled()) return;
...
bool Scene::Intersect(const GfVec3f& rayOrigin,
                      const GfVec3f& rayDir,
                      SurfaceHit& hit,
                      const IRenderJob& job) const
{
    if (!_renderState.has_value() || _renderState->TlasNodes.empty() || job.IsCancelled()) return false;
    ...
    while (stackPtr > 0) {
        ...
        if (node.leftChild < 0) {
            ...
            for (int i = 0; i < node.instanceCount; ++i) {
                ...
                int matIdx{-1};
                if (!instance.bvh.Intersect(objRayOrigin, objRayDir, instT, instNormal, instUv,
                                            instSmoothNormal, instDpdu, instDpdv, instSmoothColor, matIdx)) {
                    continue;
                }

                if (instT >= hit.t) continue;
