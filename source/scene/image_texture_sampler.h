...
private:
    [[nodiscard]] static T _default() noexcept {
        if constexpr (std::same_as<T, float>) return 1.0f;
        else                                  return T{1.0f, 1.0f, 1.0f};
    }

    [[nodiscard]] static T _extract(const GfVec3f& rgb) noexcept {
        if constexpr (std::same_as<T, float>) return rgb[0];
        else                                  return rgb;
    }
...
private:
    [[nodiscard]] const detail::Texture* _getOrLoad(std::string_view resolvedPath,
                                                     bool             linear) const;
