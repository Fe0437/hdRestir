...
HdRestirMaterial::HdRestirMaterial(SdfPath const &id) : HdMaterial(id) {}

HdRestirMaterial::~HdRestirMaterial() = default;

// Helper to walk upstream and process nodes
static void _ProcessNodeUpstream(const HdMaterialNetwork2 &network, const SdfPath &nodePath, std::set<SdfPath> &visited,
                                 HdRestirMaterial *material, TfToken targetInput = TfToken())
{
    if (visited.count(nodePath))
        return;
    ...
    if (!sdrEntry)
        sdrEntry = sdrRegistry.GetShaderNodeByIdentifier(node.nodeTypeId);

    TfToken shaderId = sdrEntry ? sdrEntry->GetIdentifier() : node.nodeTypeId;

    HdRestir_LOG << "[Restir]     Node: " << nodePath.GetText() << " | Resolved ID: " << shaderId.GetText()
                 << " | Type: " << node.nodeTypeId.GetText() << " | Target: " << targetInput.GetText() << std::endl;
    ...
    else if (shaderId == TfToken("ND_standard_surface_surfaceshader") || shaderId == TfToken("standard_surface") ||
             shaderId == TfToken("StandardSurface"))
    {
        ...
        for (auto const &param : node.parameters)
        {
            if ((param.first == TfToken("base") || param.first == TfToken("base_weight")) &&
                param.second.IsHolding<float>())
            {
            ...
            else if ((param.first == TfToken("specular") || param.first == TfToken("specular_weight")) &&
                     param.second.IsHolding<float>())
            {
            ...
            else if ((param.first == TfToken("specular_roughness") || param.first == TfToken("roughness")) &&
                     param.second.IsHolding<float>())
            {
                material->SetRoughness(param.second.UncheckedGet<float>());
            }
            ...
            else if ((param.first == TfToken("transmission") || param.first == TfToken("transmission_weight")) &&
                     param.second.IsHolding<float>())
            {
            ...
            else if ((param.first == TfToken("subsurface") || param.first == TfToken("subsurface_weight")) &&
                     param.second.IsHolding<float>())
            {
            ...
            else if ((param.first == TfToken("sheen") || param.first == TfToken("sheen_weight")) &&
                     param.second.IsHolding<float>())
            {
            ...
            else if ((param.first == TfToken("coat") || param.first == TfToken("coat_weight")) &&
                     param.second.IsHolding<float>())
            {
            ...
            else if ((param.first == TfToken("coat_IOR") || param.first == TfToken("coat_ior")) &&
                     param.second.IsHolding<float>())
            {
            ...
            else if ((param.first == TfToken("emission") || param.first == TfToken("emission_weight")) &&
                     param.second.IsHolding<float>())
            {
    ...
    else if (shaderId == TfToken("ND_open_pbr_surface_surfaceshader") || shaderId == TfToken("open_pbr_surface"))
    {
        float   base = 1.0f;
        GfVec3f baseColor(0.8f);
        for (auto const &param : node.parameters)
    ...
    else if (shaderId == TfToken("UsdUVTexture") || shaderId == TfToken("ND_image_color3") ||
             shaderId == TfToken("ND_image") || shaderId == TfToken("ND_image_float") ||
             shaderId == TfToken("ND_image_vector2") || shaderId == TfToken("ND_image_vector3") ||
             shaderId == TfToken("ND_image_vector4") || shaderId == TfToken("ND_image_color4"))
    {

        // Only map to diffuse texture if we are on a path leading to diffuse color
        if (targetInput == TfToken("diffuseColor") || targetInput == TfToken("base_color") ||
            targetInput == TfToken("base"))
        {
            for (auto const &param : node.parameters)
            {
                    ...
                    param.second.IsHolding<SdfAssetPath>())
                {
                    ...
                    HdRestir_LOG << "[Restir]       Mapped diffuse texture: "
                                 << material->GetDiffuseTexture().GetAssetPath() << std::endl;
                }
            }
        }
        else if (targetInput == TfToken("metalness") || targetInput == TfToken("base_metalness"))
        {
            for (auto const &param : node.parameters)
            {
                    ...
                    param.second.IsHolding<SdfAssetPath>())
                {
                    ...
                    HdRestir_LOG << "[Restir]       Mapped metallic texture: "
                                 << material->GetMetallicTexture().GetAssetPath() << std::endl;
                }
            }
        }
        else if (targetInput == TfToken("specular_roughness") || targetInput == TfToken("roughness"))
        {
            for (auto const &param : node.parameters)
            {
                    ...
                    param.second.IsHolding<SdfAssetPath>())
                {
                    ...
                    HdRestir_LOG << "[Restir]       Mapped roughness texture: "
                                 << material->GetRoughnessTexture().GetAssetPath() << std::endl;
                }
            }
        }
        else if (targetInput == TfToken("emissiveColor") || targetInput == TfToken("emission_color") ||
                 targetInput == TfToken("emission") || targetInput == TfToken("emission_luminance"))
        {
            for (auto const &param : node.parameters)
            {
                if ((param.first == TfToken("file") || param.first == TfToken("texcoord")) &&
                    param.second.IsHolding<SdfAssetPath>())
                {
                    HdRestir_LOG << "[Restir]       Mapped emissive texture: "
                                 << param.second.UncheckedGet<SdfAssetPath>().GetAssetPath() << std::endl;
        ...
        else if (targetInput == TfToken("normal"))
        {
            for (auto const &param : node.parameters)
            {
                    ...
                    param.second.IsHolding<SdfAssetPath>())
                {
                    ...
                    HdRestir_LOG << "[Restir]       Mapped normal texture: "
                                 << material->GetNormalTexture().GetAssetPath() << std::endl;
                }
            }
        }
    }
...
void HdRestirMaterial::Sync(HdSceneDelegate *sceneDelegate, HdRenderParam *renderParam, HdDirtyBits *dirtyBits)
{
    ...
    if (*dirtyBits & HdMaterial::DirtyResource)
    {
        ...
        if (materialResource.IsHolding<HdMaterialNetworkMap>())
        {
            ...
            for (auto const &t : network.terminals)
                HdRestir_LOG << " " << t.first.GetText();
            HdRestir_LOG << std::endl;

            // Find the best terminal
            TfToken selectedTerminal;
            TfToken terminalPriorities[] = {TfToken("mtlx:surface"), HdMaterialTerminalTokens->surface,
            ...
            if (!selectedTerminal.IsEmpty())
            {
                ...
                HdRestir_LOG << "[Restir]   Selected terminal: " << selectedTerminal.GetText() << " -> "
                             << terminalPath.GetText() << std::endl;
                _ProcessNodeUpstream(network, terminalPath, visited, this);

                HdRestir_LOG << "[Restir]   Final Params: Diffuse=" << _params.DiffuseColor
                             << " | Emission=" << (_params.EmissionColor * _params.Emission)
                             << " (Color=" << _params.EmissionColor << ", Weight=" << _params.Emission
                             << ") | Opacity=" << _params.Opacity << " | Transmission=" << _params.Transmission
                             << " (Color=" << _params.TransmissionColor << ")" << std::endl;
            }
            else
            {
    ...
    if (materialChanged)
    {
        ...
        restirRenderParam->EditScene([&](Restir::Scene &scene) { scene.SetMaterialParams(GetId(), GetParams()); });
    }

    *dirtyBits = HdMaterial::Clean;
}

HdDirtyBits HdRestirMaterial::GetInitialDirtyBitsMask() const
{
