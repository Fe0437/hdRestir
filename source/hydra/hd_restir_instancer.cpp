...
HdRestirInstancer::HdRestirInstancer(HdSceneDelegate* delegate, SdfPath const& id)
    : HdInstancer(delegate, id)
{
}

void
HdRestirInstancer::Sync(HdSceneDelegate *sceneDelegate,
...
VtMatrix4dArray
HdRestirInstancer::ComputeInstanceTransforms(SdfPath const &prototypeId)
{
    {
        std::lock_guard<std::mutex> lock(_cacheMutex);
    ...
    if (!parentInstancerId.IsEmpty()) {
        HdRestirInstancer* parentInstancer = static_cast<HdRestirInstancer*>(
