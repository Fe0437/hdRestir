...
const TfTokenVector HdRestirRenderDelegate::SUPPORTED_SPRIM_TYPES = {
    HdPrimTypeTokens->camera,       HdPrimTypeTokens->extComputation, HdPrimTypeTokens->material,
    HdPrimTypeTokens->distantLight, HdPrimTypeTokens->sphereLight,    HdPrimTypeTokens->domeLight,
...
static void _RenderCallback(Restir::Renderer *renderer, HdRenderThread *renderThread, HdRestirRenderDelegate *delegate)
{
    renderer->Clear();
...
HdRestirRenderDelegate::HdRestirRenderDelegate() : HdRenderDelegate()
{
    _Initialize();
}

HdRestirRenderDelegate::HdRestirRenderDelegate(HdRenderSettingsMap const &settingsMap) : HdRenderDelegate(settingsMap)
{
    _Initialize();
}

void HdRestirRenderDelegate::_Initialize()
{
    _sceneVersion.store(0);
    _renderParam = std::make_shared<HdRestirRenderParam>(&_scene, &_sceneVersion);

    _renderThread.SetRenderCallback(std::bind(_RenderCallback, &_renderer, &_renderThread, this));
...
HdRenderParam *HdRestirRenderDelegate::GetRenderParam() const
{
    return _renderParam.get();
}

const TfTokenVector &HdRestirRenderDelegate::GetSupportedRprimTypes() const
{
    return SUPPORTED_RPRIM_TYPES;
}

const TfTokenVector &HdRestirRenderDelegate::GetSupportedSprimTypes() const
{
    return SUPPORTED_SPRIM_TYPES;
}

const TfTokenVector &HdRestirRenderDelegate::GetSupportedBprimTypes() const
{
    return SUPPORTED_BPRIM_TYPES;
}

TfTokenVector HdRestirRenderDelegate::GetShaderSourceTypes() const
{
    return {TfToken("mtlx"), TfToken("UsdPreviewSurface"), TfToken("preview")};
}

TfTokenVector HdRestirRenderDelegate::GetMaterialRenderContexts() const
{
    return {TfToken("mtlx")};
}
...
HdRenderPassSharedPtr HdRestirRenderDelegate::CreateRenderPass(HdRenderIndex           *index,
                                                               HdRprimCollection const &collection)
{
    return HdRenderPassSharedPtr{new HdRestirRenderPass(index, collection, &_renderThread, &_renderer, &_sceneVersion)};
}

HdInstancer *HdRestirRenderDelegate::CreateInstancer(HdSceneDelegate *delegate, SdfPath const &id)
{
...
void HdRestirRenderDelegate::DestroyInstancer(HdInstancer *instancer)
{
    RemoveInstancer(instancer->GetId());
    delete instancer;
}

HdRprim *HdRestirRenderDelegate::CreateRprim(TfToken const &typeId, SdfPath const &rprimId)
{
...
void HdRestirRenderDelegate::DestroyRprim(HdRprim *rPrim)
{
    if (rPrim)
    {
...
HdSprim *HdRestirRenderDelegate::CreateSprim(TfToken const &typeId, SdfPath const &sprimId)
{
    ...
    else if (typeId == HdPrimTypeTokens->distantLight || typeId == HdPrimTypeTokens->sphereLight ||
...
HdSprim *HdRestirRenderDelegate::CreateFallbackSprim(TfToken const &typeId)
{
...
void HdRestirRenderDelegate::DestroySprim(HdSprim *sPrim)
{
    if (sPrim)
    {
...
HdBprim *HdRestirRenderDelegate::CreateBprim(TfToken const &typeId, SdfPath const &bprimId)
{
...
HdBprim *HdRestirRenderDelegate::CreateFallbackBprim(TfToken const &typeId)
{
...
void HdRestirRenderDelegate::DestroyBprim(HdBprim *bPrim)
{
    delete bPrim;
}

void HdRestirRenderDelegate::CommitResources(HdChangeTracker *tracker) {}

HdAovDescriptor HdRestirRenderDelegate::GetDefaultAovDescriptor(TfToken const &name) const
{
...
HdRenderSettingDescriptorList HdRestirRenderDelegate::GetRenderSettingDescriptors() const
{
...
VtValue HdRestirRenderDelegate::GetRenderSetting(TfToken const &key) const
{
...
void HdRestirRenderDelegate::SetRenderSetting(TfToken const &key, VtValue const &value)
{
...
void HdRestirRenderDelegate::AddMesh(const SdfPath &id, HdRestirMesh *mesh)
{
    _scene.AddMesh(id, mesh);
}

void HdRestirRenderDelegate::RemoveMesh(const SdfPath &id)
{
    _scene.RemoveMesh(id);
}

void HdRestirRenderDelegate::AddLight(const SdfPath &id, HdRestirLight *light)
{
...
void HdRestirRenderDelegate::RemoveLight(const SdfPath &id)
{
    _scene.RemoveLightFactoryInput(id);
}

void HdRestirRenderDelegate::AddInstancer(const SdfPath &id, HdRestirInstancer *instancer)
{
    _scene.AddInstancer(id, instancer);
}

void HdRestirRenderDelegate::RemoveInstancer(const SdfPath &id)
{
    _scene.RemoveInstancer(id);
}

void HdRestirRenderDelegate::AddMaterial(const SdfPath &id, HdRestirMaterial *material)
{
    _scene.SetMaterialParams(id, material->GetParams());
}

void HdRestirRenderDelegate::RemoveMaterial(const SdfPath &id)
{
