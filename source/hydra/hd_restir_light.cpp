...
HdRestirLight::HdRestirLight(SdfPath const &id, TfToken const &lightType) : HdLight(id)
{
    if (lightType == HdPrimTypeTokens->distantLight)
        _lightType = Restir::LightType::Distant;
    else if (lightType == HdPrimTypeTokens->domeLight)
        _lightType = Restir::LightType::Dome;
    else if (lightType == HdPrimTypeTokens->rectLight)
        _lightType = Restir::LightType::Rect;
    else
        _lightType = Restir::LightType::Point;
}

void HdRestirLight::Sync(HdSceneDelegate *sceneDelegate, HdRenderParam *renderParam, HdDirtyBits *dirtyBits)
{
    ...
    if (*dirtyBits & HdLight::DirtyParams)
    {
        ...
        HdRestir_LOG << "[Restir] Syncing light " << id.GetText() << " (type: " << static_cast<int>(_lightType)
                     << "):" << std::endl;
        HdRestir_LOG << "[Restir]   Intensity: " << _params.Intensity << " | Exposure: " << _params.Exposure
                     << " | Final: " << GetIntensity() << std::endl;
        if (_params.ShapingConeAngle < 180.0f)
        {
            HdRestir_LOG << "[Restir]   Shaping: Angle=" << _params.ShapingConeAngle
                         << " | Softness=" << _params.ShapingConeSoftness << std::endl;
        }
    }

    auto *restirRenderParam{static_cast<HdRestirRenderParam *>(renderParam)};
    restirRenderParam->EditScene(
        [&](Restir::Scene &scene)
        {
...
HdDirtyBits HdRestirLight::GetInitialDirtyBitsMask() const
{
