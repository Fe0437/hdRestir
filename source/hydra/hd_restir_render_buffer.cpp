...
bool
HdRestirRenderBuffer::Allocate(GfVec3i const& dimensions,
                               HdFormat format,
                               bool multiSampled)
{
...
void
HdRestirRenderBuffer::_Deallocate()
{
    std::lock_guard<std::mutex> lock(_bufferMutex);
    _width = 0;
    _height = 0;
...
void
HdRestirRenderBuffer::Write(GfVec3i const& pixel, size_t numComponents, float const* value)
{
    std::lock_guard<std::mutex> lock(_bufferMutex);
    if (pixel[0] < 0 || pixel[0] >= (int)_width || 
        pixel[1] < 0 || pixel[1] >= (int)_height) return;
    size_t idx = pixel[1] * _width + pixel[0];
    if (idx >= _renderBuffer.Count()) return;
    uint8_t *dst = _renderBuffer.Data() + idx * _renderBuffer.Stride();
    _WriteOutput(_format, dst, numComponents, value);
}

void
HdRestirRenderBuffer::Write(GfVec3i const& pixel, size_t numComponents, int const* value)
{
    std::lock_guard<std::mutex> lock(_bufferMutex);
    if (pixel[0] < 0 || pixel[0] >= (int)_width || 
        pixel[1] < 0 || pixel[1] >= (int)_height) return;
...
void
HdRestirRenderBuffer::GetFloatBuffer(std::vector<float>& outFloats) const
{
...
void
HdRestirRenderBuffer::CopyFromFrameBuffer(const Restir::FrameBuffer& source)
{
    std::lock_guard<std::mutex> lock(_bufferMutex);
    ...
    if (componentFormat != HdFormatFloat32 &&
        componentFormat != HdFormatFloat16 &&
        componentFormat != HdFormatUNorm8 &&
        componentFormat != HdFormatSNorm8 &&
...
void
HdRestirRenderBuffer::WriteSample(GfVec3i const& pixel, GfVec4f const& color)
{
    std::lock_guard<std::mutex> lock(_bufferMutex);
    if (pixel[0] < 0 || pixel[0] >= (int)_width ||
...
void
HdRestirRenderBuffer::ResolveBucket(unsigned int startX, unsigned int startY, unsigned int endX, unsigned int endY)
{
    if (_buffer.Empty() || _buffer.SizeBytes() != _renderBuffer.SizeBytes()) return;
...
void
HdRestirRenderBuffer::Clear(size_t numComponents, float const* value)
{
    std::lock_guard<std::mutex> lock(_bufferMutex);
...
void
HdRestirRenderBuffer::Clear(size_t numComponents, int const* value)
{
    std::lock_guard<std::mutex> lock(_bufferMutex);
    // Do not visually clear the buffer to prevent black flashes, just reset the accumulators
    _accumBuffer.assign(_width * _height * 4, 0.0f);
