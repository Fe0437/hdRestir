...
namespace Restir
{
    ...
    namespace
    {
        ...
        [[nodiscard]] PixelT SampleNearest(const std::vector<PixelT> &src, int srcWidth, int srcHeight, int dstX,
                                           int dstY, int dstWidth, int dstHeight)
        {
            ...
            const std::size_t srcIndex{static_cast<std::size_t>(srcY) * static_cast<std::size_t>(srcWidth) +
                                       static_cast<std::size_t>(srcX)};
            return src[srcIndex];
        }

        template <typename PixelT>
        void UpscaleBuffer(FrameBuffersMap &buffers, std::string_view name, int srcWidth, int srcHeight, int dstWidth,
                           int dstHeight)
        {
            if (!buffers.Has(name))
            {
            ...
            const std::size_t   dstCount{static_cast<std::size_t>(dstWidth) * static_cast<std::size_t>(dstHeight)};
            ...
            for (int y{0}; y < dstHeight; ++y)
            {
                ...
                {
                    const std::size_t idx{static_cast<std::size_t>(y) * static_cast<std::size_t>(dstWidth) +
