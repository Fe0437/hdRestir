...
public:
    ...
    explicit DebugOverlayPass(Config config = {})
    ...
    [[nodiscard]] std::vector<std::string> Inputs()  const override { return StaticInputs();  }
    [[nodiscard]] std::vector<std::string> Outputs() const override { return StaticOutputs(); }
...
private:
    void _drawText(gsl::span<GfVec4f> fb, int w, int h,
                   int x, int y, std::string_view text, GfVec4f color, int scale = 1);
    void _drawChar(gsl::span<GfVec4f> fb, int w, int h,
