...
std::vector<float> PostProcess::_packRgb(gsl::span<const GfVec4f> framebuffer)
{
...
void PostProcess::_applyLensFlare(std::vector<float> &finalColor, gsl::span<const float> color, unsigned int width,
                                  unsigned int height)
{
            ...
            for (int d = -blurSize; d <= blurSize; d += 2)
            {
                ...
                if (static_cast<int>(x) + d >= 0 && static_cast<int>(x) + d < static_cast<int>(width))
                {
                    const std::size_t bIdx =
                        (static_cast<std::size_t>(y) * width + static_cast<unsigned int>(static_cast<int>(x) + d)) * 3;
                ...
                {
                    const std::size_t bIdx = (static_cast<std::size_t>(static_cast<int>(y) + d) * width +
                                              static_cast<unsigned int>(static_cast<int>(x) + d)) *
                                             3;
...
void PostProcess::_applyChromaticAberration(std::vector<float> &finalColor, unsigned int width, unsigned int height,
...
void PostProcess::_applyPostProcessAlgorithm(std::vector<float> &color, unsigned int width, unsigned int height,
                                             const Config &config)
{
...
void PostProcess::Run(Restir::IFrameBufferTarget &colorBuffer, unsigned int width, unsigned int height,
                      const Config &config)
{
    if (width == 0 || height == 0)
    {
...
void PostProcess::Run(gsl::span<GfVec4f> framebuffer, int width, int height, const Config &config)
{
    if (width <= 0 || height <= 0)
    {
        return;
