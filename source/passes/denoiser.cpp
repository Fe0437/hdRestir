...
std::vector<float> Denoiser::_packRgb(gsl::span<const pxr::GfVec4f> framebuffer)
{
...
void Denoiser::_writeRgb(gsl::span<pxr::GfVec4f> framebuffer, gsl::span<const float> color)
{
    for (std::size_t i{0}; i < framebuffer.size(); ++i)
    {
...
namespace
{
    ...
    {
        return {reinterpret_cast<const float *>(pixels.data()), pixels.size() * 3};
    }

} // namespace

std::string Denoiser::_toOidnImageName(std::string_view name)
{
...
void Denoiser::_applyChromaticityBlur(std::vector<float> &prefiltered, gsl::span<const float> color, unsigned int width,
                                      unsigned int height)
{
...
void Denoiser::_runOidnDenoiser(std::vector<float> &output, gsl::span<const float> prefiltered,
...
void Denoiser::_applyDenoiserAlgorithm(std::vector<float> &color, gsl::span<const GuideBufferView> guideBuffers,
...
void Denoiser::Run(gsl::span<pxr::GfVec4f> framebuffer, gsl::span<const GuideBufferView> guideBuffers, int width,
                   int height, int frameCount, const Config &config)
{
    ...
    _applyDenoiserAlgorithm(color, guideBuffers, static_cast<unsigned int>(width), static_cast<unsigned int>(height),
                            frameCount, config);

    _writeRgb(framebuffer, color);
}

void Denoiser::Run(gsl::span<pxr::GfVec4f> framebuffer, int width, int height, const Config &config)
{
    if (width <= 0 || height <= 0)
    {
        return;
