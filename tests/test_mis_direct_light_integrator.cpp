...
public:
    explicit DummyMaterial(const GfVec3f& emission)
        : _emission{emission}
    {}
    ...
    [[nodiscard]] std::unique_ptr<Restir::IBSDF> CreateBSDF(Restir::BSDFClosure&& /*c*/) const override
    {
        ...
        const Restir::ShadingPoint& /*surface*/,
...
public:
    ...
    [[nodiscard]] std::optional<Restir::LightCandidate> ProposeCandidate(
...
public:
    explicit DummyScene(const Restir::IMaterial* material)
        : _material{material}
    {}

    std::recursive_mutex& GetSceneLock() override { return _sceneLock; }

    void BuildRenderState(const Restir::SceneBuildRenderStateConfig& /*config*/, const Restir::IRenderJob& /*job*/) override {}

    [[nodiscard]] const Restir::IMaterial& GetMaterial(int /*matId*/) const override { return *_material; }
    ...
    [[nodiscard]] const Restir::ILight* GetLightAtHit(const Restir::HitRecord& /*hit*/) const override { return nullptr; }

    [[nodiscard]] std::optional<Restir::HitRecord> IntersectScene(
...
void TestPendingConnectionIsConsumedAndReset()
{
    ...
    const std::optional<Restir::BsdfBounceConnection> connection{Restir::BsdfBounceConnection{
        .Bounce = Restir::BsdfBounceSample{
